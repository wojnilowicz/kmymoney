...
if [ -f $KMYMONEY_INSTALL_PREFIX/lib/x86_64-linux-gnu/plugins/sqldrivers/qsqlcipher.so ]; then
  echo "Copying qsqlcipher for tests..."
  cp -fv $KMYMONEY_INSTALL_PREFIX/lib/x86_64-linux-gnu/plugins/sqldrivers/qsqlcipher.so $DEPS_INSTALL_PREFIX/plugins/sqldrivers
fi
...
if [ -d $DEPS_INSTALL_PREFIX/share/libofx ]; then
  echo "Copying libofx..."
  rsync -prul $DEPS_INSTALL_PREFIX/share/libofx $KMYMONEY_INSTALL_PREFIX/share
fi
...
if [ -d $DEPS_INSTALL_PREFIX/share/aqbanking ]; then
  ...
  rsync -prul $DEPS_INSTALL_PREFIX/lib/gwenhywfar/plugins/79/* $PLUGINS/gwenhywfar
  rsync -prul $DEPS_INSTALL_PREFIX/lib/aqbanking/plugins/44/* $PLUGINS/aqbanking
fi

if [ -f $DEPS_INSTALL_PREFIX/lib/libKF5Wallet.so.5 ]; then
  echo "Copying KWallet..."
  rsync -prul $DEPS_INSTALL_PREFIX/lib/libKF5Wallet* $KMYMONEY_INSTALL_PREFIX/lib
  rsync -prul $DEPS_INSTALL_PREFIX/lib/libkwalletbackend* $KMYMONEY_INSTALL_PREFIX/lib
  rsync -prul $DEPS_INSTALL_PREFIX/bin/kwallet* $KMYMONEY_INSTALL_PREFIX/bin
  rsync -prul $DEPS_INSTALL_PREFIX/share/kservices5/kwallet* $KMYMONEY_INSTALL_PREFIX/share/kservices5
fi
...
$DOWNLOADS_DIR/linuxdeployqt-continuous-x86_64.AppImage \
  ...
  -executable=$KMYMONEY_INSTALL_PREFIX/bin/kwalletd5 \
  -executable=$KMYMONEY_INSTALL_PREFIX/bin/kwallet-query \
...
# workaround a bug in linuxdeployqt
# https://discourse.appimage.org/t/libqt5core-bloated/1702/5
LIBQT5CORE_FILENAME="$KMYMONEY_INSTALL_PREFIX/lib/libQt5Core.so.5"
echo "Size of libQt5Core.so.5 before replacement `du -h $LIBQT5CORE_FILENAME | cut -f1`"
cp -Lfv $DEPS_INSTALL_PREFIX/lib/libQt5Core.so.5 $LIBQT5CORE_FILENAME
patchelf --set-rpath '$ORIGIN' $LIBQT5CORE_FILENAME
strip $LIBQT5CORE_FILENAME
echo "Size of libQt5Core.so.5 after replacement `du -h $LIBQT5CORE_FILENAME | cut -f1`"
...
patchelf --set-rpath '$ORIGIN' $KMYMONEY_INSTALL_PREFIX/lib/libgpg-error.so.0
...
# upload on GitHub
# bash $KMYMONEY_SOURCES/packaging/common/upload.sh "KMyMoneyNEXT%20for%20Linux" "${APPIMAGE_NAME}.AppImage"
