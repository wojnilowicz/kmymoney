...
createDMG () {
  ...
  # hdiutil makehybrid gives smaller intermediate image but final image is the same size
...
if [ -d $DEPS_INSTALL_PREFIX/share/libofx ]; then
  echo "Copying libofx..."
  rsync -prul $DEPS_INSTALL_PREFIX/share/libofx $CONTENTSDIR/Resources
fi
...
if [ -f $DEPS_INSTALL_PREFIX/lib/libKF5Wallet.dylib ]; then
  echo "Copying KWallet..."
  rsync -prul $DEPS_INSTALL_PREFIX/lib/libKF5Wallet* $CONTENTSDIR/Frameworks
  rsync -prul $DEPS_INSTALL_PREFIX/lib/libkwallet* $CONTENTSDIR/Frameworks
  
  find $DEPS_INSTALL_PREFIX/lib/libexec/kf5
#   rsync -prul $DEPS_INSTALL_PREFIX/bin/kwallet* $CONTENTSDIR/MacOS
#   rsync -prul $DEPS_INSTALL_PREFIX/share/kservices5/kwallet* $CONTENTSDIR/Resources/kservices5
fi
...
if [ -d $DEPS_INSTALL_PREFIX/share/aqbanking ]; then
  ...
  rsync -prul $DEPS_INSTALL_PREFIX/lib/gwenhywfar/plugins/79/* $CONTENTSDIR/PlugIns/gwenhywfar
  rsync -prul $DEPS_INSTALL_PREFIX/lib/aqbanking/plugins/44/* $CONTENTSDIR/PlugIns/aqbanking
fi
...
# upload on GitHub
bash $KMYMONEY_SOURCES/packaging/common/upload.sh "KMyMoneyNEXT%20for%20macOS" "$DMG_NAME"