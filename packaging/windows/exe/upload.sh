...
if [ ! -z "$UPLOADTOOL_SUFFIX" ] ; then
  if [ "$UPLOADTOOL_SUFFIX" = "$CI_TAG" ] ; then
    RELEASE_NAME="$CI_TAG"
    RELEASE_TITLE="Release build ($CI_TAG)"
    is_prerelease="false"
  else
...
if [ "$ARTIFACTORY_BASE_URL" != "" ]; then
  ...
  for file in ${files[@]}; do
    url="${ARTIFACTORY_BASE_URL}/travis-${CI_BUILD_NUMBER}/"$(basename "$file")
...
# if [ "$CI_TAG" != "$CI_BRANCH" ] && [ "$CI_BRANCH" != "master" ]; then export TRAVIS_EVENT_TYPE=pull_request; fi
...
#    --data '{"commit_id": "'"$CI_COMMIT"'","body": "'"$body"'","event": "COMMENT"}' \
#    $review_url)
#  if echo $review_comment | grep -q "Bad credentials" 2>/dev/null ; then
#    echo '"Bad credentials" response for --data {"commit_id": "'"$CI_COMMIT"'","body": "'"$body"'","event": "COMMENT"}'
#  fi
  $shatool "$@"
  exit 0
fi
...
if [ ! -z "$CI_REPO_SLUG" ] ; then
  ...
  # We are not running on supported CI
  echo "Not running on supported CI"
  if [ -z "$REPO_SLUG" ] ; then
...
if [ "$CI_COMMIT" != "$target_commit_sha" ] ; then

  echo "CI_COMMIT != target_commit_sha, hence deleting $RELEASE_NAME..."
  ...
  release_infos=$(curl -H "Authorization: token ${GITHUB_TOKEN}" \
       --data '{"tag_name": "'"$RELEASE_NAME"'","target_commitish": "'"$CI_COMMIT"'","name": "'"$RELEASE_TITLE"'","body": "'"$BODY"'","draft": false,"prerelease": '$is_prerelease'}' "https://api.github.com/repos/$REPO_SLUG/releases")
