...
unsigned int ReaderWriter::fileVersionRead = 0;
unsigned int ReaderWriter::fileVersionWrite = 0;
...
public:
  MyMoneyXmlContentHandler(ReaderWriter* reader);
...
MyMoneyXmlContentHandler::MyMoneyXmlContentHandler(ReaderWriter* reader) :
    m_reader(reader),
    m_loc(0),
    m_level(0),
    m_elementCount(0)
...
void ReaderWriter::readFile(const QByteArray &rawStorage, MyMoneyFile &file)
{
  ...
  QBuffer rawStorageDevice;
  rawStorageDevice.setData(rawStorage);
  QXmlInputSource xml(&rawStorageDevice);
...
QByteArray ReaderWriter::writeFile(const MyMoneyFile &file)
{
  m_file = &const_cast<MyMoneyFile &>(file);

  // qDebug("XMLWRITER: Starting file write");
  m_doc = new QDomDocument(tagName(Tag::File));
  Q_CHECK_PTR(m_doc);
  ScopeHelper<QDomDocument> helper(&m_doc);
  ...
  QDomElement mainElement = m_doc->createElement(tagName(Tag::File));
...
bool ReaderWriter::readFileInformation(const QDomElement& fileInfo)
{
...
void ReaderWriter::writeFileInformation(QDomElement& fileInfo)
{
  ...
  version.setAttribute(attributeName(Attribute::General::ID), "20200101");
...
void ReaderWriter::writeUserInformation(QDomElement& userInfo)
{
...
bool ReaderWriter::readUserInformation(const QDomElement& userElement)
{
  bool rc = true;
...
void ReaderWriter::writeInstitutions(QDomElement& parent)
{
...
void ReaderWriter::writeInstitution(QDomElement& institution, const MyMoneyInstitution& i)
{
  MyMoneyXmlContentHandler::writeInstitution(i, *m_doc, institution);
}

void ReaderWriter::writePayees(QDomElement& parent)
{
...
void ReaderWriter::writePayee(QDomElement& payee, const MyMoneyPayee& p)
{
  MyMoneyXmlContentHandler::writePayee(p, *m_doc, payee);
}

void ReaderWriter::writeTags(QDomElement& parent)
{
...
void ReaderWriter::writeTag(QDomElement& tag, const MyMoneyTag& ta)
{
  MyMoneyXmlContentHandler::writeTag(ta, *m_doc, tag);
}

void ReaderWriter::writeAccounts(QDomElement& accounts)
{
...
void ReaderWriter::writeAccount(QDomElement& account, const MyMoneyAccount& p)
{
  MyMoneyXmlContentHandler::writeAccount(p, *m_doc, account);
}

void ReaderWriter::writeTransactions(QDomElement& transactions)
{
...
void ReaderWriter::writeTransaction(QDomElement& transaction, const MyMoneyTransaction& tx)
{
  MyMoneyXmlContentHandler::writeTransaction(tx, *m_doc, transaction);
}

void ReaderWriter::writeSchedules(QDomElement& parent)
{
...
void ReaderWriter::writeSchedule(QDomElement& scheduledTx, const MyMoneySchedule& tx)
{
  MyMoneyXmlContentHandler::writeSchedule(tx, *m_doc, scheduledTx);
}

void ReaderWriter::writeSecurities(QDomElement& parent)
{
...
void ReaderWriter::writeSecurity(QDomElement& securityElement, const MyMoneySecurity& security)
{
  MyMoneyXmlContentHandler::writeSecurity(security, *m_doc, securityElement);
}

void ReaderWriter::writeCurrencies(QDomElement& parent)
{
  SecuritiesModel::xmlWriter writer(&MyMoneyXmlContentHandler::writeSecurity, *m_doc, parent);
...
void ReaderWriter::writeReports(QDomElement& parent)
{
...
void ReaderWriter::writeReport(QDomElement& report, const MyMoneyReport& r)
{
  MyMoneyXmlContentHandler2::writeReport(r, *m_doc, report);
}

void ReaderWriter::writeBudgets(QDomElement& parent)
{
...
void ReaderWriter::writeBudget(QDomElement& budget, const MyMoneyBudget& b)
{
  MyMoneyXmlContentHandler2::writeBudget(b, *m_doc, budget);
}

void ReaderWriter::writeOnlineJobs(QDomElement& parent)
{
...
void ReaderWriter::writeOnlineJob(QDomElement& onlineJobs, const onlineJob& job)
{
  MyMoneyXmlContentHandler::writeOnlineJob(job, *m_doc, onlineJobs);
}

void ReaderWriter::writeCostCenters(QDomElement& parent)
{
...
void ReaderWriter::writeCostCenter(QDomElement& costCenters, const MyMoneyCostCenter& costCenter)
{
  MyMoneyXmlContentHandler::writeCostCenter(costCenter, *m_doc, costCenters);
}

QDomElement ReaderWriter::findChildElement(const QString& name, const QDomElement& root)
{
...
QDomElement ReaderWriter::writeKeyValuePairs(const QMap<QString, QString> pairs)
{
  if (m_doc) {
...
void ReaderWriter::writePrices(QDomElement& prices)
{
  QString from;
  QString to;
...
void ReaderWriter::setProgressCallback(void(*callback)(int, int, const QString&))
{
  m_progressCallback = callback;
}

void ReaderWriter::signalProgress(int current, int total, const QString& msg)
{
