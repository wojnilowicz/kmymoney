...
#include "storagexmlreaderwriter20190203-test.h"
...
QTEST_GUILESS_MAIN(Storage::Xml::ReaderWriter20190203Test)

namespace Storage {
namespace Xml {

const QString filesDirectory = QStringLiteral("./xmlFiles/KMyMoney/2019-02-03/");

void ReaderWriter20190203Test::readBasicFile()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    QCOMPARE("John Doe", storage->user().name());
  };

  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicFile.kmy"), testStorage);
}

void ReaderWriter20190203Test::readBasicPayee()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicPayee.kmy"), testStorage);
}

void ReaderWriter20190203Test::readBasicInstitution()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
...
//    QCOMPARE(institution.manager(), "manager");
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicInstitution.kmy"), testStorage);
}

void ReaderWriter20190203Test::readBasicAccount()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicAccount.kmy"), testStorage);
}

void ReaderWriter20190203Test::readBasicTransaction()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicTransaction.kmy"), testStorage);
}

void ReaderWriter20190203Test::readBasicTag()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("basicTag.kmy"), testStorage);
}
void ReaderWriter20190203Test::readBasicScheduledTransaction()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  const auto originalFilePath = filesDirectory + QStringLiteral("basicSchedule.kmy");
  ...
  const auto configuredFilePath = ReaderWriterHelper::configureFile(originalFilePath, replacementDatas);
  ReaderWriterHelper::testFile(configuredFilePath, testStorage);
}

void ReaderWriter20190203Test::overdueScheduledTransaction()
{
  ...
  std::function<void(const MyMoneyFile * const)> overdueTestStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    const auto scheduledTX = storage->schedule(scheduledTXID);
    QVERIFY(scheduledTX.isOverdue());
  };

  std::function<void(const MyMoneyFile * const)> ontimeTestStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    const auto scheduledTX = storage->schedule(scheduledTXID);
    QVERIFY(!scheduledTX.isOverdue());
  };

  const auto originalFilePath = filesDirectory + QStringLiteral("overdueSchedule.kmy");
  ...
  // replace certain dates in the source file for proper testing
  auto configuredFilePath = ReaderWriterHelper::configureFile(originalFilePath, overdueReplacementDatas);
  ReaderWriterHelper::testFile(configuredFilePath, overdueTestStorage);
  ...
  configuredFilePath = ReaderWriterHelper::configureFile(originalFilePath, ontimeReplacementDatas);
  ReaderWriterHelper::testFile(configuredFilePath, ontimeTestStorage);
}

void ReaderWriter20190203Test::nextPayment()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    const auto scheduledTX = storage->schedule(scheduledTXID);
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("nextPayment.kmy"), testStorage);
}

void ReaderWriter20190203Test::nextPaymentOnLastDayOfMonth()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("nextPaymentOnLastDayOfMonth.kmy"), testStorage);
}

void ReaderWriter20190203Test::paymentDates()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    auto scheduledTX = storage->schedule(scheduledTXID);
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("paymentDates.kmy"), testStorage);
}

void ReaderWriter20190203Test::hasReferenceInSchedule()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    const auto scheduledTX = storage->schedule(scheduledTXID);
  ...
  const auto originalFilePath = filesDirectory + QStringLiteral("hasReferenceInSchedule.kmy");
...
void ReaderWriter20190203Test::paidEarlyOneTime()
{
  ...
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    ...
    QCOMPARE(scheduledTX.occurrence(), Schedule::Occurrence::Monthly);
    QCOMPARE(scheduledTX.paymentDates(QDate::currentDate(), QDate::currentDate().addDays(21)).count(), 0);
  };

  const auto originalFilePath = filesDirectory + QStringLiteral("paidEarlyOneTime.kmy");
...
void ReaderWriter20190203Test::replaceIDinSchedule()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    const auto scheduledTXID = QStringLiteral("SCH000001");
    auto scheduledTX = storage->schedule(scheduledTXID);
  ...
  const auto originalFilePath = filesDirectory + QStringLiteral("replaceIDinSchedule.kmy");
...
void ReaderWriter20190203Test::readMergedTransaction()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
  ...
  ReaderWriterHelper::testFile(filesDirectory + QStringLiteral("mergedTransaction.kmy"), testStorage);
}

void ReaderWriter20190203Test::replaceIDinSplit()
{
  std::function<void(const MyMoneyFile * const)> testStorage = [] (const MyMoneyFile * const storage) {
    QList<MyMoneyTransaction> transactionsList;
    MyMoneyTransactionFilter transactionsFilter;
...
//    file->attachStorage(const_cast<MyMoneyFile *>(storage));
