...
{
  QSKIP( "Superseeded by xmlFile20181201Test::readMergedTransaction");
  MyMoneyTransaction t;
...
void MyMoneyXmlContentHandlerTest::writeTransaction()
{
  ...
  s.setBankID("SPID");
  s.addMatch();
  s.setValue("kmm-match-transaction", QString::fromLatin1("%1;%2").arg(t.id(), matchedTx.id()));
  s.setValue("kmm-match-split", QString::fromLatin1("%1;%2").arg(s.id(), matchedTx.splits().at(0).id()));
  QVERIFY(s.matchedTransaction() == matchedTx.id());
...
{
  QSKIP( "Superseeded by xmlFile20181201Test::replaceIDinSplit");
  MyMoneySplit s;
  bool changed;
