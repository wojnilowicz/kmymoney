...
MyMoneyStorageSql::MyMoneyStorageSql(const QSqlDatabase &db) :
  QSqlDatabase(db),
  d_ptr(new MyMoneyStorageSqlPrivate(this))
{
  Q_D(MyMoneyStorageSql);
  d->m_driver = MyMoneyDbDriver::create(driverName());
}
...
void MyMoneyStorageSql::setManager(MyMoneyFile *mgr)
{
  Q_D(MyMoneyStorageSql);
  d->m_file = mgr;
}

bool MyMoneyStorageSql::readFile(MyMoneyFile &storageManager)
{
  Q_D(MyMoneyStorageSql);
  d->m_file = &storageManager;
  d->m_loadAll = true;
  d->m_file->setFileFixVersion(5);
...
bool MyMoneyStorageSql::writeFile(const MyMoneyFile &storageManager)
{
  Q_D(MyMoneyStorageSql);
  d->m_file = &const_cast<MyMoneyFile &>(storageManager);
  d->m_dbVersion = 20200101;
...
void MyMoneyStorageSql::initializeDatabase()
{
  Q_D(MyMoneyStorageSql);
  d->createTables();
}
